...
function LoginView({ onLogin }) { const [userId, setUserId] = useState('master'); const user = authUsers.find((item) => item.id === userId); return <div className="login-screen"><div className="login-card"><div className="brand-mark"><span /></div><span className="eyebrow">PROJECTTRACK ACCESS</span><h1>Sign in to your workspace</h1><p>Choose the account scope for this session. Project and notification access follows the selected role.</p><label><span>User type</span><select value={userId} onChange={(e) => setUserId(e.target.value)}>{authUsers.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.role}</option>)}</select></label><div className="access-preview"><strong>{user.role}</strong><span>{user.role === 'Master Login' ? 'All projects and all system data' : user.role === 'Project Manager' ? 'Managed projects, schedules, resources and notifications' : 'Own projects, tasks and personal notifications'}</span></div><button className="primary-button login-button" onClick={() => onLogin(user)}>Continue as {user.name}</button></div></div>; }
function AppShell({ session, setSession }) {
  ...
  return <div className="app-shell">
    <header className="titlebar">
      ...
      <div className="title-actions"><button aria-label="Help"><HelpCircle size={16} /></button><button aria-label="Notifications" onClick={() => setActiveNav('Notifications')}><Bell size={16} /><i /></button><div className="avatar">SS</div></div>
    </header>
...
const authUsers = [
  { id: 'maya', name: 'Maya Patel', role: 'Individual', initials: 'MP', projects: ['b2c'] },
  { id: 'sandy', name: 'Sandy Sudhir', role: 'Project Manager', initials: 'SS', projects: ['orion', 'atlas', 'delta'] },
  { id: 'master', name: 'Master Login', role: 'Master Login', initials: 'ML', projects: projects.slice(1).map((project) => project.id) },
];
