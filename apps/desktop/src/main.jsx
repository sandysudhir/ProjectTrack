...
function AppShell({ session, setSession }) {
  ...
  const addChildActivity = (parent) => { const name = window.prompt(`Add activity under ${parent.name}`); if (!name?.trim()) return; const nextId = Math.max(0, ...allProjectRows.map((item) => Number(item.id) || 0), ...customRows.map((item) => Number(item.id) || 0)) + 1; const projectId = parent.projectId || (parent.id <= 4 ? 'orion' : parent.id <= 7 ? 'atlas' : parent.id <= 10 ? 'delta' : 'b2c'); setCustomRows((current) => [...current, { id: nextId, parentId: parent.id, projectId, name: name.trim(), level: (parent.level || 0) + 1, type: 'task', start: parent.start, finish: parent.finish, bar: parent.bar, duration: parent.duration, status: 'Not started', progress: 0, color: 'blue' }]); setNotice(`${name.trim()} added under ${parent.name}`); };
