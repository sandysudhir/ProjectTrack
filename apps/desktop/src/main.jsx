...
function ScheduleView({ rows, leftWidth, setLeftWidth, density, setDensity, zoom, setZoom, outlineMode, setOutlineMode, action, notice, activeTool, onCreateBar, onToggleCollapse, onAddChild, collapsedIds }) {
  ...
  const beginSplit = (event) => { event.preventDefault(); event.currentTarget.setPointerCapture?.(event.pointerId); setSplitDragging(true); };
  const updateSplit = (event) => { if (!splitDragging || !scheduleBodyRef.current) return; const rect = scheduleBodyRef.current.getBoundingClientRect(); const next = Math.max(24, Math.min(68, ((event.clientX - rect.left) / rect.width) * 100)); setLeftWidth(next); };
