...
if (initialParams.get('reset') === '1') {
  ['projecttrack-workspace', 'projecttrack-custom-rows', 'projecttrack-revised-bars', 'projecttrack-progress', 'projecttrack-rationales', 'projecttrack-links', 'projecttrack-last-save'].forEach((key) => localStorage.removeItem(key));
...
function ProjectFilesView({ projectId, mode, files, onAddFile, onAddComment, notice }) {
...
function App() {
  ...
  const filteredRows = useMemo(() => applyFilter(scopedRows.filter((row) => !query || row.name.toLowerCase().includes(query.toLowerCase())), filterName), [query, filterName, scopedRows]);
