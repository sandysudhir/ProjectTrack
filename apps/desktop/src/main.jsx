...
import {
  ...
  Table2, Undo2, Users, X, GraduationCap, CheckCircle2
} from 'lucide-react';
import './styles.css';
...
const navItems = [
  ...
  ['Dashboard', LayoutDashboard], ['My Tasks', ClipboardList], ['Learning Plans', GraduationCap], ['Tags', ListFilter], ['Notifications', Bell], ['Documents', ClipboardList], ['Comments', MessageCircle], ['Attachments', Paperclip], ['Risks', AlertTriangle], ['Issues', CircleHelp]
];

const teamLearningPlans = [
  { id: 'sandy', name: 'Sandy Sudhir', role: 'Project Manager', initials: 'SS', activities: ['Portfolio governance', 'Change control'], plan: [{ id: 'sandy-1', title: 'Stakeholder communication refresh', type: 'Course', source: 'Project leadership', status: 'Complete' }, { id: 'sandy-2', title: 'Schedule risk review exercise', type: 'Practice', source: 'Portfolio governance', status: 'In progress' }], tests: [{ id: 'sandy-t1', title: 'Project controls check', score: '92%', status: 'Passed' }], links: [{ title: 'Project management fundamentals', url: 'https://www.youtube.com/watch?v=QX7b4f9QJ4c' }] },
  { id: 'maya', name: 'Maya Patel', role: 'Industrial Designer', initials: 'MP', activities: ['Industrial design', 'Concept development'], plan: [{ id: 'maya-1', title: 'Presentation layout and visual hierarchy', type: 'Feedback', source: 'Manager feedback · customer presentation layout', status: 'In progress' }, { id: 'maya-2', title: 'SolidWorks CAD essentials', type: 'Auto-added', source: 'Core activity · CAD work', status: 'Not started' }], tests: [{ id: 'maya-t1', title: 'SolidWorks modeling assessment', score: 'Pending', status: 'Assigned' }, { id: 'maya-t2', title: 'Customer presentation layout review', score: 'Pending', status: 'Assigned' }], links: [{ title: 'SolidWorks beginner tutorial', url: 'https://www.youtube.com/results?search_query=solidworks+beginner+tutorial' }, { title: 'Presentation visual hierarchy', url: 'https://www.youtube.com/results?search_query=presentation+layout+visual+hierarchy' }] },
  { id: 'arjun', name: 'Arjun Mehta', role: 'Systems Architect', initials: 'AM', activities: ['Architecture', 'Schematic design'], plan: [{ id: 'arjun-1', title: 'Architecture decision records', type: 'Course', source: 'Architecture', status: 'Complete' }, { id: 'arjun-2', title: 'Schematic review checklist', type: 'Practice', source: 'Manager feedback · review quality', status: 'In progress' }], tests: [{ id: 'arjun-t1', title: 'Architecture review assessment', score: '78%', status: 'Passed' }], links: [{ title: 'Systems architecture patterns', url: 'https://www.youtube.com/results?search_query=systems+architecture+patterns' }] },
  { id: 'neha', name: 'Neha Kulkarni', role: 'Firmware Engineer', initials: 'NK', activities: ['Firmware', 'Core development', 'Integration'], plan: [{ id: 'neha-1', title: 'Embedded testing and debugging', type: 'Auto-added', source: 'Core activity · firmware integration', status: 'In progress' }, { id: 'neha-2', title: 'Release handover checklist', type: 'Course', source: 'Release readiness', status: 'Not started' }], tests: [{ id: 'neha-t1', title: 'Firmware integration test', score: 'Pending', status: 'Assigned' }], links: [{ title: 'Embedded systems debugging', url: 'https://www.youtube.com/results?search_query=embedded+systems+debugging+tutorial' }] },
];
...
function NotificationsView({ events, notice }) { return <section className="notifications-view"><div className="placeholder-toolbar"><button className="toolbar-button active"><Bell size={15} /> Notifications</button><span className="range-label">Activity audit · {events.length} events</span></div><div className="notification-list">{events.map((event) => <article className="notification-card" key={event.id}><div className="notification-avatar">{event.actor.slice(0,2).toUpperCase()}</div><div><strong>{event.actor} {event.action} {event.forMe && <b className="for-you-badge">FOR YOU</b>}</strong><span>{event.detail}</span><time>{event.time}</time></div></article>)}</div><div className="panel-note"><span className="note-icon"><Bell size={14} /></span><div><strong>{notice}</strong><span>Changes, comments, documents, links, tasks and uploads are recorded in this activity feed.</span></div></div></section>; }
...
function AutofillDialog({ rows, onClose, onGenerate, seedTarget }) {
  const selectableRows = seedTarget ? [seedTarget, ...rows] : rows;
  const [targetId, setTargetId] = useState(String(seedTarget?.id || selectableRows.find((row) => row.type === 'summary')?.id || selectableRows[0]?.id || ''));
  const [hours, setHours] = useState('160');
  const [area, setArea] = useState('Prototyping');
  const [resource, setResource] = useState('Systems Architect');
  const [allocation, setAllocation] = useState('50%');
  const [proposal, setProposal] = useState('');
  const target = selectableRows.find((row) => String(row.id) === targetId);
  const templates = { Prototyping: ['Release drawings', 'Vendor discussions and final quotes', 'Prototype build and fund transfer', 'Ship from China', 'Inspection and acceptance'], Conceptualization: ['Research and discovery', 'Concept sketches', 'Design review', 'Concept approval'], 'CAD / SolidWorks': ['Set up CAD standards', '3D part modelling', 'Assembly and drawings', 'Design review and revisions'], Testing: ['Test plan', 'Prepare fixtures', 'Execute tests', 'Document results'] };
  const extractProposal = (file) => { if (!file) return; const reader = new FileReader(); reader.onload = () => setProposal(String(reader.result || '').slice(0, 5000)); reader.readAsText(file); };
  return <div className="dialog-backdrop" role="presentation" onClick={onClose}><div className="autofill-dialog" role="dialog" aria-modal="true" aria-label="Autofill schedule" onClick={(e) => e.stopPropagation()}><div className="dialog-title"><strong>Autofill schedule</strong><button onClick={onClose} aria-label="Close autofill"><X size={16} /></button></div><div className="dialog-tabs"><button className="active">Plan generator</button><button>Proposal extraction</button><button>Resources</button></div><div className="autofill-grid"><label><span>Parent step / project</span><select value={targetId} onChange={(e) => setTargetId(e.target.value)}>{selectableRows.map((row) => <option key={row.id} value={row.id}>{row.name}</option>)}</select></label><label><span>Total planned hours</span><input type="number" min="1" value={hours} onChange={(e) => setHours(e.target.value)} /></label><label><span>Generic work area</span><select value={area} onChange={(e) => setArea(e.target.value)}>{Object.keys(templates).map((item) => <option key={item}>{item}</option>)}</select></label><label><span>Primary resource</span><select value={resource} onChange={(e) => setResource(e.target.value)}><option>Systems Architect</option><option>Industrial Designer</option><option>Application Developer</option><option>Project Manager</option><option>Business Analyst</option></select></label><label><span>Allocation</span><select value={allocation} onChange={(e) => setAllocation(e.target.value)}><option>25%</option><option>50%</option><option>100%</option></select></label></div><div className="autofill-template"><strong>Generated steps</strong><span>{templates[area].join(' · ')} · durations scaled to {hours} hours</span></div><label className="proposal-upload"><span>Upload customer proposal (text-readable)</span><input type="file" accept=".txt,.md,.csv,.xml,.json" onChange={(e) => extractProposal(e.target.files?.[0])} /></label>{proposal && <textarea className="proposal-preview" value={proposal} onChange={(e) => setProposal(e.target.value)} placeholder="Extracted proposal text" />}{proposal && <div className="proposal-hint">Proposal text is ready for review; generated steps will be added beneath the selected parent.</div>}<div className="dialog-actions"><button className="ghost-button" onClick={onClose}>Cancel</button><button className="primary-button" onClick={() => onGenerate({ target, hours: Number(hours) || 160, area, resource, allocation, templates: templates[area], proposal })}>Generate plan</button></div></div></div>;
}


function ChildActivityDialog({ parent, onClose, onCreate }) {
  const [name, setName] = useState('');
  return <div className="dialog-backdrop" role="presentation" onClick={onClose}><form className="new-row-dialog" role="dialog" aria-modal="true" aria-label="Add child activity" onClick={(event) => event.stopPropagation()} onSubmit={(event) => { event.preventDefault(); if (name.trim()) onCreate(name.trim()); }}><div className="dialog-title"><strong>Add activity under {parent.name}</strong><button type="button" onClick={onClose} aria-label="Close add activity"><X size={16} /></button></div><p className="dialog-hint">This activity will fold into {parent.name} and inherit its schedule dates.</p><label><span>Activity name</span><input autoFocus value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. Vendor review" /></label><div className="dialog-actions"><button type="button" className="ghost-button" onClick={onClose}>Cancel</button><button type="submit" className="primary-button" disabled={!name.trim()}>Add activity</button></div></form></div>;
}

function ProjectDialog({ onClose, onCreate }) {
  const [name, setName] = useState('');
  return <div className="dialog-backdrop" role="presentation"><div className="new-row-dialog" role="dialog" aria-modal="true" aria-label="Add project"><div className="dialog-title"><strong>Add project</strong><button onClick={onClose} aria-label="Close add project"><X size={16} /></button></div><label><span>Project name</span><input autoFocus value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. Phoenix Rollout" /></label><div className="dialog-actions"><button className="ghost-button" onClick={onClose}>Cancel</button><button className="ghost-button" disabled={!name.trim()} onClick={() => onCreate(name.trim(), true)}>Add & autofill</button><button className="primary-button" disabled={!name.trim()} onClick={() => onCreate(name.trim(), false)}>Add project</button></div></div></div>;
}
...
function LoginView({ onLogin }) {
  ...
  const allowed = [user.email, ...(user.alternateEmails || [])];
  const submit = (provider) => { const normalized = email.trim().toLowerCase(); if (!allowed.includes(normalized)) { setError(`Use an authorized company email for this account: ${allowed.join(' or ')}`); return; } setError(''); onLogin({ ...user, loginProvider: provider, loginEmail: normalized }); };
  return <div className="login-screen"><div className="login-card"><div className="brand-mark"><span /></div><span className="eyebrow">PROJECTTRACK ACCESS</span><h1>Sign in to your workspace</h1><p>Only company-authorized email IDs can access this workspace.</p><label><span>User type</span><select value={userId} onChange={(e) => { setUserId(e.target.value); setError(''); }}>{authUsers.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.role}</option>)}</select></label><label><span>Authorized email</span><input type="email" value={email} onChange={(e) => { setEmail(e.target.value); setError(''); }} placeholder={user.email} /></label>{error && <div className="login-error">{error}</div>}<div className="access-preview"><strong>{user.role}</strong><span>{user.role === 'Master Login' ? 'All projects and all system data' : user.role === 'Project Manager' ? 'Managed projects, schedules, resources and notifications' : 'Own projects, tasks and personal notifications'}</span></div><button className="primary-button login-button" onClick={() => submit('email')}>Continue with email</button><button className="google-login" onClick={() => submit('google')}>G&nbsp;&nbsp;Continue with Google</button><small className="login-allowed">Authorized for this account: {allowed.join(' · ')}</small></div></div>;
}
...
function AppShell({ session, setSession }) {
  ...
  const [childDialogParent, setChildDialogParent] = useState(null);
  ...
  const [events, setEvents] = useState(() => { try { return JSON.parse(localStorage.getItem('projecttrack-events') || '[]'); } catch { return []; } });
  ...
  const [learningPlans, setLearningPlans] = useState(() => { try { return JSON.parse(localStorage.getItem('projecttrack-learning-plans') || JSON.stringify(teamLearningPlans)); } catch { return teamLearningPlans; } });
  ...
  useEffect(() => { localStorage.setItem('projecttrack-events', JSON.stringify(events)); }, [events]);
  useEffect(() => { localStorage.setItem('projecttrack-learning-plans', JSON.stringify(learningPlans)); }, [learningPlans]);
  const logEvent = (action, detail, forMe = false) => setEvents((current) => [{ id: Date.now(), actor: 'You', action, detail, forMe, time: new Date().toLocaleString() }, ...current].slice(0, 100));

  const scopedRows = useMemo(() => {
    const base = activeProject === 'all' ? allProjectRows.filter((row) => session.role === 'Master Login' || session.projects.includes(row.projectId || (row.id <= 4 ? 'orion' : row.id <= 7 ? 'atlas' : row.id <= 10 ? 'delta' : 'b2c'))) : (projectRowsById[activeProject] || []);
  ...
  const visibleRows = useMemo(() => {
    const scopedBase = outlineMode === 'compact' ? filteredRows.filter((row) => row.level === 0 || row.type === 'milestone') : outlineMode === 'selective' ? filteredRows.filter((row) => row.level <= 1 || row.type === 'milestone') : filteredRows;
    const scoped = []; const visit = (row) => { scoped.push(row); scopedBase.filter((child) => child.parentId === row.id).forEach(visit); }; scopedBase.filter((row) => row.parentId == null).forEach(visit);
    const output = [];
  ...
  const showRisks = activeNav === 'Risks';
  ...
  const showLearningPlans = activeNav === 'Learning Plans';
  const showTags = activeNav === 'Tags';
  const showNotifications = activeNav === 'Notifications';

  const availableProjects = [...projects, ...extraProjects];
  const visibleProjects = session.role === 'Master Login' ? availableProjects : [projects[0], ...availableProjects.filter((project) => session.projects.includes(project.id))];
  const addProject = (name, openAutofill = false) => { const id = `project-${Date.now()}`; const project = { id, label: name, kind: 'project', status: 'On track' }; setExtraProjects((current) => [...current, project]); setActiveProject(id); setProjectDialogOpen(false); setNotice(`${name} added`); if (openAutofill) { setAutofillTarget({ id, name, level: 0, start: '01 Sep 26', finish: '30 Sep 26', bar: [0, 29], projectId: id, type: 'summary' }); setAutofillOpen(true); } };
  const action = (label) => {
    if (label === 'Autofit') { setAutofillOpen(true); return; }
  ...
  const generateAutofill = ({ target, hours, area, resource, allocation, templates, proposal }) => { if (!target) return; const nextIdBase = Math.max(0, ...allProjectRows.map((item) => Number(item.id) || 0), ...customRows.map((item) => Number(item.id) || 0)) + 1; const perStep = Math.max(1, Math.round(hours / templates.length)); const startDay = Number(String(target.start || '01 Sep 26').match(/\d{1,2}/)?.[0] || 1); const projectId = target.projectId || (target.id <= 4 ? 'orion' : target.id <= 7 ? 'atlas' : target.id <= 10 ? 'delta' : 'b2c'); const generated = templates.map((name, index) => { const stepHours = index === templates.length - 1 ? Math.max(1, hours - perStep * (templates.length - 1)) : perStep; const days = Math.max(1, Math.ceil(stepHours / 8)); const s = Math.min(30, startDay + index * days); const f = Math.min(30, s + days - 1); return { id: nextIdBase + index, parentId: target.id, projectId, name, level: (target.level || 0) + 1, type: 'task', start: `${s} Sep 26`, finish: `${f} Sep 26`, bar: [s - 1, f - 1], duration: `${stepHours}h`, status: 'Not started', progress: 0, color: 'blue', resource, allocation, plannedHours: stepHours, proposalSource: proposal ? 'Customer proposal upload' : undefined, tags: [area] }; }); setCustomRows((current) => [...current, ...generated]); setAutofillOpen(false); setNotice(`${templates.length} ${area} steps generated under ${target.name} · ${hours}h · ${resource} at ${allocation}`); };
  ...
  const addChildActivity = (parent) => setChildDialogParent(parent);
  const createChildActivity = (name) => { const parent = childDialogParent; if (!parent) return; const nextId = Math.max(0, ...allProjectRows.map((item) => Number(item.id) || 0), ...customRows.map((item) => Number(item.id) || 0)) + 1; const projectId = parent.projectId || (parent.id <= 4 ? 'orion' : parent.id <= 7 ? 'atlas' : parent.id <= 10 ? 'delta' : 'b2c'); setCustomRows((current) => [...current, { id: nextId, parentId: parent.id, projectId, name, level: (parent.level || 0) + 1, type: 'task', start: parent.start, finish: parent.finish, bar: parent.bar, duration: parent.duration, status: 'Not started', progress: 0, color: 'blue' }]); setChildDialogParent(null); setNotice(name + ' added under ' + parent.name); };
  ...
  const addProjectFile = (file) => { const target = activeProject === 'all' ? 'orion' : activeProject; setProjectFiles((current) => [...current, { ...file, id: `file-${Date.now()}`, projectId: target }]); setNotice(`${file.kind === 'link' ? 'Link' : file.kind === 'document' ? 'Document' : 'Attachment'} added to ${projects.find((p) => p.id === target)?.label}`); logEvent(`added a ${file.kind === 'link' ? 'link' : file.kind === 'document' ? 'document' : 'attachment'}`, `${file.name} · ${projects.find((p) => p.id === target)?.label}`); };
  const addProjectComment = (comment) => { const target = activeProject === 'all' ? 'orion' : activeProject; setProjectFiles((current) => [...current, { id: `comment-${Date.now()}`, projectId: target, kind: 'comment', name: comment, meta: `Comment · ${new Date().toLocaleDateString()}` }]); setNotice(`Comment added to ${projects.find((p) => p.id === target)?.label}`); logEvent('added a comment', `${comment} · ${projects.find((p) => p.id === target)?.label}`, /@(you|sandy)\b/i.test(comment)); };
  ...
  return <div className="app-shell">
    <header className="titlebar">
      ...
      <div className="title-actions"><button aria-label="Help"><HelpCircle size={16} /></button><button aria-label="Notifications" onClick={() => setActiveNav('Notifications')}><Bell size={16} /><i /></button><button className="avatar" aria-label="Sign out" title={`Signed in as ${session.loginEmail || session.email} · Sign out`} onClick={() => { localStorage.removeItem("projecttrack-session"); setSession(null); }}>{session.initials || "SS"}</button></div>
    </header>

    <nav className="workspace-tabs" aria-label="Project tabs">
      <div className="tabs-left">{visibleProjects.map((project) => <button key={project.id} className={`workspace-tab ${activeProject === project.id ? 'active' : ''}`} onClick={() => { setActiveProject(project.id); setNotice(`${project.label} opened`); }}>{project.label}{project.id === 'all' && <span className="tab-count">{projects.length - 1}</span>}</button>)}<button className="add-tab" onClick={() => action('New project tab')}><Plus size={15} /></button></div>
    ...
    <div className="work-area">
      <aside className="sidebar">
        ...
        <div className="project-tree"><div className="tree-node root"><ChevronDown size={14} /><FolderKanban size={15} /><span>All Projects</span></div>{visibleProjects.slice(1).map((p) => <div className="tree-node" key={p.id}><ChevronRight size={14} /><FolderKanban size={15} /><span>{p.label}</span><span className={`mini-status ${p.status === 'At risk' ? 'risk' : ''}`} /></div>)}</div>
      ...
      <main className="main-content">
        <div className="view-header"><div><div className="breadcrumb">{activeProject === 'all' ? 'ALL PROJECTS' : projects.find((p) => p.id === activeProject)?.label.toUpperCase()} <ChevronRight size={13} /> {activeNav.toUpperCase()}</div><h1>{showMyTasks || showLearningPlans || showNotifications || showRisks || showFiles || showDashboard || showReports ? activeNav : showCalendar ? 'Calendar' : showResource ? 'Resource planning' : 'Schedule'}</h1><p>{showMyTasks ? 'Personal and project tasks in one workspace.' : showRisks ? 'Resource overloads and schedule risks requiring management action.' : showLearningPlans ? 'Individual education, testing and improvement plans for every contributor.' : showNotifications ? 'Audit trail of project and workspace activity.' : showFiles ? 'Project-scoped documents, links, comments and attachments.' : showDashboard ? 'Portfolio completion, schedule health and project momentum.' : showReports ? 'Visual status of delivery, completion and attention items.' : showCalendar ? 'Wall calendar projection across shared project weeks.' : showResource ? 'Resource load and assignment structure across the workspace.' : 'Plan, inspect and coordinate work across your project portfolio.'}</p></div><div className="view-actions"><button className="ghost-button" onClick={() => action('Refresh view')}><RefreshCw size={15} /> Refresh</button><button className="primary-button" onClick={() => action('New row')}><Plus size={15} /> New row</button></div></div>
        {mode === 'loading' ? <div className="state-panel"><RefreshCw size={25} className="spin" /><h2>Loading workspace</h2><p>Preparing the deterministic sample fixture…</p></div> : <>
          {showNotifications ? <NotificationsView events={session.role === 'Individual' ? events.filter((event) => event.forMe) : events} notice={notice} /> : showTags ? <TagsView rows={filteredRows} onSelect={(tag) => { setTagFilter(tag); setActiveNav('Schedule'); setNotice(tag + ' tag filter applied'); }} notice={notice} /> : showMyTasks ? <MyTasksView rows={filteredRows} notice={notice} personalTasks={personalTasks} onAddTask={(name) => { setPersonalTasks((current) => [...current, { id: `personal-${Date.now()}`, name, section: 'Personal', status: 'Open' }]); setNotice('Personal task added'); logEvent('added a personal task', name, true); }} /> : showLearningPlans ? <LearningPlansView plans={learningPlans} setPlans={setLearningPlans} notice={notice} /> : showFiles ? <ProjectFilesView projectId={activeProject} mode={activeNav} files={projectFiles} onAddFile={addProjectFile} onAddComment={addProjectComment} notice={notice} globalQuery={query} /> : showCost ? <CostLayoutView notice={notice} action={action} /> : showStatus ? <StatusLayoutView rows={filteredRows} notice={notice} /> : showDashboard ? <DashboardView rows={filteredRows} notice={notice} learningPlans={learningPlans} action={action} /> : showReports ? <ReportsView rows={filteredRows} notice={notice} /> : showCalendar ? <CalendarPlaceholder notice={notice} action={action} rows={filteredRows} projectLabel={activeProject === 'all' ? 'All Projects' : projects.find((p) => p.id === activeProject)?.label} /> : showRisks ? <RisksView notice={notice} /> : showResource ? <ResourcePlaceholder notice={notice} action={action} onOpenSchedule={(activity) => { setActiveNav('Schedule'); setNotice(activity + ' opened in Schedule for correction'); }} rows={filteredRows} projectLabel={activeProject === 'all' ? 'All Projects' : projects.find((p) => p.id === activeProject)?.label} /> : <ScheduleView rows={visibleRows} leftWidth={leftWidth} setLeftWidth={setLeftWidth} density={density} setDensity={setDensity} zoom={zoom} setZoom={setZoom} outlineMode={outlineMode} setOutlineMode={(next) => { setOutlineMode(next); setExpanded(next !== 'compact'); }} action={action} notice={notice} activeTool={activeTool} onCreateBar={(day) => createBar({ projectId: activeProject, day })} onToggleCollapse={toggleCollapse} onAddChild={addChildActivity} collapsedIds={collapsedIds} />}
        </>}
      </main>
    </div>
    ...
    {autofillOpen && <AutofillDialog rows={allProjectRows} seedTarget={autofillTarget} onClose={() => { setAutofillOpen(false); setAutofillTarget(null); }} onGenerate={generateAutofill} />}
    {childDialogParent && <ChildActivityDialog parent={childDialogParent} onClose={() => setChildDialogParent(null)} onCreate={createChildActivity} />}
...
function App() { const [session, setSession] = useState(() => { try { return JSON.parse(localStorage.getItem('projecttrack-session') || 'null'); } catch { return null; } }); return session ? <AppShell session={session} setSession={setSession} /> : <LoginView onLogin={(user) => { localStorage.setItem('projecttrack-session', JSON.stringify(user)); setSession(user); }} />; }
...
function ActivityInfoDialog({ row, onClose }) {
  const resources = ['Systems Architect', 'Industrial Designer', 'Application Developer', 'Project Manager', 'Business Analyst'];
  const [hours, setHours] = useState(() => { try { return JSON.parse(localStorage.getItem('projecttrack-activity-hours-' + row.id) || '{}'); } catch { return {}; } });
  const save = () => { localStorage.setItem('projecttrack-activity-hours-' + row.id, JSON.stringify(hours)); onClose(); };
  return <div className="dialog-backdrop" role="presentation" onClick={onClose}><div className="activity-info-dialog" role="dialog" aria-modal="true" aria-label={'Activity information for ' + row.name} onClick={(event) => event.stopPropagation()}><div className="dialog-title"><div><span className="eyebrow">ACTIVITY INFORMATION</span><strong>{row.name}</strong></div><button onClick={onClose} aria-label="Close activity information"><X size={16} /></button></div><div className="activity-info-meta"><span>Scheduled: {row.start} → {row.finish}</span><span>Duration: {row.duration}</span><span>Status: {row.status}</span></div><div className="activity-resource-head"><span>Assigned resource</span><span>Hours / day</span></div>{resources.map((resource) => <label className="activity-resource-row" key={resource}><span>{resource}</span><input type="number" min="0" max="8" step="1" value={hours[resource] ?? 0} onChange={(event) => setHours({ ...hours, [resource]: Math.max(0, Math.min(8, Number(event.target.value) || 0)) })} /></label>)}<p className="dialog-hint">Set the actual daily allocation for this activity. Zero means the resource is not working on it.</p><div className="dialog-actions"><button className="ghost-button" onClick={onClose}>Cancel</button><button className="primary-button" onClick={save}>Save resource hours</button></div></div></div>;
}

function ScheduleView({ rows, leftWidth, setLeftWidth, density, setDensity, zoom, setZoom, outlineMode, setOutlineMode, action, notice, activeTool, onCreateBar, onToggleCollapse, onAddChild, collapsedIds }) {
  ...
  const [activityInfoRow, setActivityInfoRow] = useState(null);
  ...
  const beginSplit = (event) => { event.preventDefault(); splitDraggedRef.current = false; event.currentTarget.setPointerCapture?.(event.pointerId); setSplitDragging(true); };
  const updateSplit = (event) => { if (!splitDragging || !scheduleBodyRef.current) return; splitDraggedRef.current = true; const rect = scheduleBodyRef.current.getBoundingClientRect(); const next = Math.max(24, Math.min(68, ((event.clientX - rect.left) / rect.width) * 100)); setLeftWidth(next); };
  const endSplit = () => setSplitDragging(false);
  ...
  return <><section className={'schedule-panel density-' + density.toLowerCase()}>
    ...
    <div className="schedule-body" ref={scheduleBodyRef}><div className="grid-pane" style={{ width: leftWidth + '%' }}><div className="grid-header"><span className="row-number">#</span><span className="activity-name">Activity name</span><span>Start</span><span>Finish</span><span>Dur.</span><span>Status</span><span>% Complete</span></div><div className="grid-rows">{displayRows.map((row) => <div className={'grid-row ' + (row.type === 'summary' ? 'summary ' : '') + (row.type === 'milestone' ? 'milestone-row' : '')} key={row.id} onClick={(event) => activeTool === 'Link' && handleBarClick(event, row)}><span className="row-number">{row.id}</span><span className="activity-name" style={{ paddingLeft: 'calc(10px + ' + row.level * 18 + 'px)' }}>{row.type === 'summary' ? <button className="collapse-toggle" aria-label={`${collapsedIds.has(row.id) ? 'Expand' : 'Collapse'} ${row.name}`} onClick={(event) => { event.stopPropagation(); onToggleCollapse(row); }}><ChevronDown size={13} className={collapsedIds.has(row.id) ? 'collapsed' : ''} /></button> : row.type === 'milestone' ? <Gauge size={12} className="milestone-icon" /> : <span className="task-dot" />}<span>{row.name}</span>{onAddChild && <button className="add-child" aria-label={`Add activity under ${row.name}`} onClick={(event) => { event.stopPropagation(); onAddChild(row); }}>+</button>}</span><span>{row.start}</span><span>{row.finish}</span><span>{row.duration}</span><span className={'status ' + (row.status === 'At risk' ? 'risk' : row.status === 'Complete' ? 'complete' : '')}>{row.status}</span><span className="completion-cell"><span className="completion-track"><i style={{ width: (progressOverrides[row.id] ?? row.progress) + '%' }} /></span>{progressOverrides[row.id] ?? row.progress}%</span></div>)}</div></div><button className={splitDragging ? "splitter dragging" : "splitter"} aria-label="Resize grid pane" title="Drag to resize activity and timeline panes" onClick={() => { if (splitDraggedRef.current) { splitDraggedRef.current = false; return; } setLeftWidth(leftWidth > 44 ? 34 : 52); }} onPointerDown={beginSplit} onPointerMove={updateSplit} onPointerUp={endSplit} onPointerCancel={endSplit}><span /></button><div className="timeline-pane"><div className="timeline-header"><div className="month-label">September 2026</div><div className="timeline-date-stack"><div className="weekday-labels">{timeline.map((day, i) => <span key={'w-' + day}>{['T','W','T','F','S','S','M'][i % 7]}</span>)}</div><div className="day-labels">{timeline.map((day, i) => <span key={day} className={i === 8 ? 'today' : ''}>{day}</span>)}</div></div></div><div className="timeline-grid" ref={timelineRef} onClick={handleTimelineClick} onPointerMove={updateResize} onPointerUp={finishResize} onPointerLeave={finishResize} style={{ '--scale': scale }}><div className="today-line" style={{ left: (8.5 / 30) * 100 + '%' }}><span>Today</span></div>{rows.map((row, index) => { const revised = revisedBars[row.id]; const shown = revised || row.bar; const progress = progressOverrides[row.id] ?? row.progress; const isCritical = criticalIds.includes(Number(row.id)); const isSource = linkSource === row.id; return <div key={row.id} className={'timeline-row ' + (row.type === 'summary' ? 'summary' : '')}><div className={'bar ' + row.color + ' ' + (revised ? 'has-revision ' : '') + (isCritical ? 'critical-bar ' : '') + (isSource ? 'link-source' : '') + (risks[row.id] ? 'risk-' + risks[row.id].severity : '')} style={{ left: (row.bar[0] / 30) * 100 + '%', width: row.type === 'milestone' ? '0' : ((row.bar[1] - row.bar[0] + 1) / 30) * 100 * scale + '%' }} onClick={(event) => handleBarClick(event, row)} onContextMenu={(event) => { event.preventDefault(); setRiskMenu({ row, x: event.clientX, y: event.clientY }); }} onDoubleClick={() => setActivityInfoRow(row)}>{row.type === 'milestone' ? <span className="diamond" /> : <><button className="resize-handle original-handle start" aria-label={'Adjust ' + row.name + ' start'} onPointerDown={(event) => beginResize(event, row, 'start')} /><button className="resize-handle original-handle end" aria-label={'Adjust ' + row.name + ' finish'} onPointerDown={(event) => beginResize(event, row, 'end')} /><span className="bar-progress" style={{ width: progress + '%' }} /><button className="progress-handle" aria-label={'Adjust ' + row.name + ' completion'} onPointerDown={(event) => beginProgress(event, row)} style={{ left: progress + '%' }}><span>{progress}%</span></button><span className="bar-label">{row.name}</span>{index > 0 && <span className="dependency" />}</>}</div>{revised && <div className={'bar revised ' + row.color + ' ' + (isCritical ? 'critical-bar' : '')} style={{ left: (shown[0] / 30) * 100 + '%', width: ((shown[1] - shown[0] + 1) / 30) * 100 * scale + '%' }} onClick={(event) => handleBarClick(event, row)} onDoubleClick={() => setActivityInfoRow(row)} title={rationales[row.id] ? 'Rationale: ' + rationales[row.id] : 'Double-click to add rationale'}><button className="resize-handle start" aria-label={'Adjust ' + row.name + ' start'} onPointerDown={(event) => beginResize(event, row, 'start')} /><button className="resize-handle end" aria-label={'Adjust ' + row.name + ' finish'} onPointerDown={(event) => beginResize(event, row, 'end')} /><span className="revised-progress" style={{ width: progress + '%' }} /><button className="progress-handle revised-progress-handle" aria-label={'Adjust ' + row.name + ' completion'} onPointerDown={(event) => beginProgress(event, row)} style={{ left: progress + '%' }} /><span className="revision-label">Edited · {shown[0] + 1}–{shown[1] + 1} Sep</span>{rationales[row.id] && <span className="rationale-preview">{rationales[row.id]}</span>}</div>}</div>; })}</div><div className="timeline-footer"><span>01 Sep</span><span>Week 37</span><span>08 Sep</span><span>15 Sep</span><span>22 Sep</span><span>30 Sep</span></div></div></div><div className="scroll-controls" aria-label="Schedule navigation"><button onClick={() => nudgeScroll(-420, 0)} aria-label="Scroll left">←</button><button onClick={() => nudgeScroll(0, -300)} aria-label="Scroll up">↑</button><span>Drag the bottom scrollbar or swipe to view the full timeline</span><button onClick={() => nudgeScroll(0, 300)} aria-label="Scroll down">↓</button><button onClick={() => nudgeScroll(420, 0)} aria-label="Scroll right">→</button></div><div className="quicklook-card"><span className="eyebrow">QUICKLOOK · HOVER PREVIEW</span><strong>Architecture</strong><span>07 Sep → 14 Sep · 6d · 62% complete</span><span>Resources: Systems Architect, Business Analysts</span></div><div className="panel-note"><span className="note-icon"><Clock3 size={14} /></span><div><strong>{notice}</strong><span>Hover a task bar to reveal resize handles. Drag an edge to revise dates while the original remains; drag the completion marker to change %. Select the Link tool to sequence two activities and then run the deterministic scheduler.</span></div><button onClick={() => runScheduler()}>Details</button></div></section>{riskMenu && <div className="risk-context-menu" style={{ left: riskMenu.x, top: riskMenu.y }} role="menu"><strong>Mark risk · {riskMenu.row.name}</strong><span>Risk points</span><div className="risk-point-actions"><button onClick={() => markRisk(riskMenu.row, 'medium', 3)}>3</button><button onClick={() => markRisk(riskMenu.row, 'medium', 7)}>7</button><button onClick={() => markRisk(riskMenu.row, 'severe', 10)}>10 · Severe</button></div><button onClick={() => markRisk(riskMenu.row, 'none', 0)}>Clear risk</button></div>}{reasonFor && <div className="dialog-backdrop" role="presentation"><div className="rationale-dialog" role="dialog" aria-modal="true" aria-label="Revision rationale"><div className="dialog-title"><strong>Revision rationale</strong><button onClick={() => setReasonFor(null)} aria-label="Close rationale"><X size={16} /></button></div><p>Describe why the revised timeline changed. The original bar remains visible beneath the edited line.</p><textarea value={draftReason} onChange={(e) => setDraftReason(e.target.value)} placeholder="Enter rationale for the extension…" autoFocus /><div className="dialog-actions"><button className="ghost-button" onClick={() => setReasonFor(null)}>Cancel</button><button className="primary-button" onClick={saveRationale}>Save rationale</button></div></div></div>}{activityInfoRow && <ActivityInfoDialog row={activityInfoRow} onClose={() => setActivityInfoRow(null)} />}</>;
}
function LearningPlansView({ plans, setPlans, notice }) {
  const [selected, setSelected] = useState(plans[0]?.id || '');
  const [testDraft, setTestDraft] = useState({ title: '', app: '', url: '' });
  const [boards, setBoards] = useState(() => { try { return JSON.parse(localStorage.getItem('projecttrack-innovation-boards') || '{}'); } catch { return {}; } });
  const [ideaDraft, setIdeaDraft] = useState({ title: '', notes: '', access: 'Private' });
  const person = plans.find((item) => item.id === selected) || plans[0];
  const togglePlan = (itemId) => setPlans((current) => current.map((member) => member.id !== person.id ? member : { ...member, plan: member.plan.map((item) => item.id === itemId ? { ...item, status: item.status === 'Complete' ? 'Not started' : 'Complete' } : item) }));
  const addTest = () => { if (!testDraft.title.trim()) return; setPlans((current) => current.map((member) => member.id !== person.id ? member : { ...member, tests: [...member.tests, { id: `${member.id}-t-${Date.now()}`, title: testDraft.title.trim(), app: testDraft.app.trim() || 'External test app', url: testDraft.url.trim(), score: 'Pending', status: 'Assigned' }] })); setTestDraft({ title: '', app: '', url: '' }); };
  const boardItems = boards[person?.id] || [];
  const addIdea = () => { if (!ideaDraft.title.trim()) return; const item = { id: `idea-${Date.now()}`, title: ideaDraft.title.trim(), notes: ideaDraft.notes.trim() || 'Concept and next steps to be defined.', access: ideaDraft.access, created: new Date().toLocaleDateString() }; setBoards((current) => ({ ...current, [person.id]: [item, ...(current[person.id] || [])] })); setIdeaDraft({ title: '', notes: '', access: 'Private' }); };
  useEffect(() => { localStorage.setItem('projecttrack-innovation-boards', JSON.stringify(boards)); }, [boards]);
  const completed = person ? person.plan.filter((item) => item.status === 'Complete').length : 0;
  const total = person ? person.plan.length : 0;
  return <section className="learning-view"><div className="learning-header"><div><span className="eyebrow">PEOPLE · LEARNING & IMPROVEMENT</span><h2>Individual development plans</h2><p>Each contributor has a single place for required learning, tests, videos and manager feedback actions.</p></div><div className="learning-progress"><strong>{total ? Math.round(completed / total * 100) : 0}%</strong><span>{completed}/{total} plan items complete</span></div></div><div className="learning-layout"><aside className="member-list"><strong>Team members</strong>{plans.map((member) => <button key={member.id} className={member.id === person?.id ? 'active' : ''} onClick={() => setSelected(member.id)}><span className="member-avatar">{member.initials}</span><span><b>{member.name}</b><small>{member.role}</small></span><em>{member.plan.filter((item) => item.status === 'Complete').length}/{member.plan.length}</em></button>)}</aside><div className="learning-detail"><div className="member-hero"><div className="member-avatar large">{person?.initials}</div><div><h3>{person?.name}</h3><span>{person?.role}</span><small>Core activities: {person?.activities.join(' · ')}</small></div></div><div className="learning-columns"><div className="learning-panel"><div className="panel-title"><h3>Improvement plan</h3><span>Auto-added from activities and feedback</span></div>{person?.plan.map((item) => <article className="learning-item" key={item.id}><button aria-label={`Mark ${item.title} complete`} onClick={() => togglePlan(item.id)}><CheckCircle2 size={17} className={item.status === 'Complete' ? 'checked' : ''} /></button><div><strong>{item.title}</strong><span>{item.type} · {item.source}</span></div><em className={item.status === 'Complete' ? 'complete-text' : ''}>{item.status}</em></article>)}</div><div className="learning-panel"><div className="panel-title"><h3>Tests</h3><span>Required checks</span></div>{person?.tests.map((test) => <article className="learning-item test-item" key={test.id}><div><strong>{test.title}</strong><span>{test.status} · {test.app || 'External test app'}</span></div><em>{test.score}</em>{test.url && <a href={test.url} target="_blank" rel="noreferrer" className="test-link">Open ↗</a>}</article>)}<div className="test-form"><input value={testDraft.title} onChange={(e) => setTestDraft({ ...testDraft, title: e.target.value })} placeholder="Standard test name" /><input value={testDraft.app} onChange={(e) => setTestDraft({ ...testDraft, app: e.target.value })} placeholder="Testing app / provider" /><input value={testDraft.url} onChange={(e) => setTestDraft({ ...testDraft, url: e.target.value })} placeholder="Result or test link (optional)" /><button className="primary-button" onClick={addTest}>Add test</button></div></div><div className="learning-panel"><div className="panel-title"><h3>Training links</h3><span>Open resources</span></div>{person?.links.map((link) => <a className="learning-link" href={link.url} target="_blank" rel="noreferrer" key={link.url}><span><Play size={14} />{link.title}</span><small>Open video/resource ↗</small></a>)}</div></div><div className="innovation-board"><div className="panel-title"><div><h3>Blue-sky innovation whiteboard</h3><span>Private by default · ideas, experiments and self-improvement activities</span></div><span>{boardItems.length} ideas</span></div><div className="idea-form"><input value={ideaDraft.title} onChange={(e) => setIdeaDraft({ ...ideaDraft, title: e.target.value })} placeholder="New idea or improvement activity" /><textarea value={ideaDraft.notes} onChange={(e) => setIdeaDraft({ ...ideaDraft, notes: e.target.value })} placeholder="Concept, next steps, technology to explore…" /><div><select value={ideaDraft.access} onChange={(e) => setIdeaDraft({ ...ideaDraft, access: e.target.value })}><option>Private</option><option>Share with management team</option><option>Share with teammates</option></select><button className="primary-button" onClick={addIdea}>Add to whiteboard</button></div></div><div className="idea-grid">{boardItems.map((idea) => <article className="idea-card" key={idea.id}><div><strong>{idea.title}</strong><span>{idea.notes}</span></div><footer><em>{idea.access}</em><small>{idea.created}</small></footer></article>)}{boardItems.length === 0 && <div className="empty-files">Your private whiteboard is empty. Capture an idea, experiment or personal activity.</div>}</div><div className="connected-spaces"><span className="eyebrow">CONNECTED TEAM SPACES</span>{plans.filter((member) => member.id !== person?.id).map((member) => <span key={member.id}><i className="member-avatar">{member.initials}</i>{member.name}</span>)}</div></div></div></div><div className="panel-note"><span className="note-icon"><GraduationCap size={14} /></span><div><strong>{notice}</strong><span>Plans are seeded from assigned activities; add tests and private innovation ideas, then share a whiteboard explicitly when collaboration is needed.</span></div></div></section>;
}
function DashboardView({ rows, notice, learningPlans, action }) {
  ...
  const trainingComplete = learningPlans.filter((member) => member.plan.every((item) => item.status === 'Complete')).length;
  return <section className="dashboard-view"><div className="dashboard-hero"><div><span className="eyebrow">PORTFOLIO DASHBOARD</span><h2>All projects at a glance</h2><p>Overall completion, schedule health and project momentum across the portfolio.</p></div><div className="completion-ring" style={{ background: `conic-gradient(#1779d1 0 ${overall}%, #dbe7f2 ${overall}% 100%)` }}><strong>{overall}%</strong><span>complete</span></div></div><div className="dashboard-cards"><div><span>Projects</span><strong>{projectRows.length}</strong><small>Active portfolio</small></div><div><span>Activities</span><strong>{rows.length}</strong><small>Across all plans</small></div><div><span>At risk</span><strong>{projectRows.reduce((s,p) => s+p.risk,0)}</strong><small>Needs attention</small></div><div><span>Learning plans</span><strong>{trainingComplete}/{learningPlans.length}</strong><small>Contributors complete</small></div></div><div className="project-health-grid">{projectRows.map((project) => <article key={project.id}><div className="health-title"><strong>{project.label}</strong><span className={project.risk ? 'risk' : 'good'}>{project.risk ? `${project.risk} at risk` : 'On track'}</span></div><div className="health-track"><i style={{ width: `${project.progress}%` }} /></div><div className="health-meta"><span>{project.progress}% complete</span><span>{project.count} activities</span></div></article>)}</div><div className="dashboard-learning"><div className="panel-title"><h3>Contributor improvement plans</h3><span>Project manager view</span></div>{learningPlans.map((member) => { const done = member.plan.filter((item) => item.status === 'Complete').length; return <div className="dashboard-learning-row" key={member.id}><span className="member-avatar">{member.initials}</span><strong>{member.name}</strong><span>{member.role}</span><div className="mini-track"><i style={{ width: `${Math.round(done / member.plan.length * 100)}%` }} /></div><b>{done}/{member.plan.length} complete</b></div>; })}</div><div className="dashboard-resource-alert"><div className="panel-title"><h3>Resource load summary</h3><span>Organization-wide</span></div><p><strong className="risk-text">2 resources exceed 100%</strong>. Review Systems Architect (128%) and Contractor Procurement (118%).</p><button className="toolbar-button" onClick={() => action("Risks")}>Review overload risks</button></div><div className="panel-note"><span className="note-icon"><LayoutDashboard size={14} /></span><div><strong>{notice}</strong><span>Portfolio completion and contributor learning completion are visible together for project managers.</span></div></div></section>;
}
...
const resourceLoads = [
  { name: 'Systems Architect', category: 'People', overall: 128, hours: 205, projects: [['Orion Program', 80], ['On2Cook B2C', 48]], activities: ['Architecture', 'Schematic design', 'Design review'] },
  { name: 'Industrial Designer', category: 'People', overall: 92, hours: 147, projects: [['On2Cook B2C', 62], ['Atlas Program', 30]], activities: ['Concept development', 'Design freeze', 'Prototype review'] },
  { name: 'Application Developer', category: 'People', overall: 105, hours: 168, projects: [['On2Cook.AI', 70], ['Delta Release', 35]], activities: ['Core development', 'Integration', 'Build and validation'] },
  { name: 'Business Analyst', category: 'People', overall: 64, hours: 102, projects: [['Orion Program', 40], ['Atlas Program', 24]], activities: ['Feasibility', 'Discovery', 'Requirements'] },
  { name: 'Project Manager', category: 'People', overall: 78, hours: 125, projects: [['Orion Program', 35], ['Atlas Program', 25], ['Delta Release', 18]], activities: ['Portfolio governance', 'Change control', 'Release readiness'] },
  { name: 'Contractor Procurement', category: 'External', overall: 118, hours: 189, projects: [['Atlas Program', 68], ['On2Cook B2C', 50]], activities: ['Vendor discussions and final quotes', 'Purchase orders', 'Prototype sourcing'] },
];
const authUsers = [
  { id: 'maya', name: 'Maya Patel', role: 'Individual', initials: 'MP', email: 'maya@inventindia.com', projects: ['b2c'] },
  { id: 'sandy', name: 'Sandy Sudhir', role: 'Project Manager', initials: 'SS', email: 'sandy@inventindia.com', alternateEmails: ['sandy@ontobook.com', 'sandy.inventindia@gmail.com'], projects: ['orion', 'atlas', 'delta'] },
  { id: 'master', name: 'Master Login', role: 'Master Login', initials: 'ML', email: 'admin@inventindia.com', projects: projects.slice(1).map((project) => project.id) },
];
function ResourcePlaceholder({ action, notice, onOpenSchedule, projectLabel = 'All Projects', rows = [] }) {
  ...
  const dayNumber = (value) => { const match = String(value || '').match(/(\d{1,2})/); return match ? Number(match[1]) : null; };
  const resourceData = scoped.map((resource) => {
  ...
  const overloaded = resourceData.filter((r) => r.daily.some((hours) => hours > 8));
  const selected = selectedActivity ? resourceData.flatMap((r) => r.assignments.map((a) => ({ ...a, resource: r.name }))).find((a) => a.activity === selectedActivity) : null;
  return <section className="resource-view"><div className="placeholder-toolbar"><button className="toolbar-button active"><Users size={15} /> Daily resource schedule</button><label className="workweek-control">Work week <select value={workWeek} onChange={(e) => setWorkWeek(e.target.value)}><option value="5-day">5-day · Sat/Sun off</option><option value="6-day">6-day · Sun off</option></select></label><span className="range-label">{projectLabel} · daily capacity tracker · 8h baseline</span></div><div className="resource-summary"><div><span>Average daily load</span><strong>{Math.round(resourceData.reduce((sum, r) => sum + r.daily.reduce((a,b) => a+b,0) / Math.max(1,r.daily.length),0) / Math.max(1,resourceData.length))} h</strong></div><div><span>Overloaded resources</span><strong className={overloaded.length ? 'risk-text' : ''}>{overloaded.length}</strong></div><div><span>Schedule-linked activities</span><strong>{resourceData.reduce((sum,r) => sum + r.assignments.length, 0)}</strong></div></div><div className="resource-grid-wrap"><div className="resource-grid-head"><span>Resource / assigned activity</span>{days.map((day, i) => <span className={workingDay(i) ? '' : 'nonworking-head'} key={day}>{day}<small>{workingDay(i) ? '' : 'OFF'}</small></span>)}</div>{resourceData.map((resource) => <div className="resource-group" key={resource.name}><div className="resource-group-title"><strong>{resource.name}</strong><span>{resource.category} · {resource.overall}% overall · {resource.hours} planned h</span></div><div className="resource-total-row"><b>Daily utilization</b>{resource.daily.map((hours, i) => <span className={!workingDay(i) ? 'nonworking-cell' : hours > 8 ? 'overload-cell' : hours ? 'loaded-cell' : ''} key={days[i]}>{hours}h<br/><small>{Math.round(hours / 8 * 100)}%</small></span>)}</div>{resource.assignments.map((assignment) => <div className="resource-assignment-row" key={assignment.activity}><button className="resource-activity-link" onClick={() => setSelectedActivity(assignment.activity)}><span>↳ {assignment.activity}</span><small>{assignment.hours}h/day · schedule linked · View schedule ↗</small></button>{days.map((_, i) => <span className={!workingDay(i) ? 'nonworking-cell' : i + 1 >= assignment.start && i + 1 <= assignment.finish ? 'assignment-cell' : ''} key={i}>{i + 1 >= assignment.start && i + 1 <= assignment.finish ? `${assignment.hours}h` : ''}</span>)}</div>)}<div className="resource-load-track"><i style={{ width: `${Math.min(100, resource.overall)}%` }} /><b>{resource.overall}% engagement</b></div>{resource.daily.some((hours) => hours > 8) && <div className="overload-note"><AlertTriangle size={14} /> Daily usage exceeds 8 hours. Rebalance assignments; this is a schedule risk.</div>}</div>)}<div className="resource-schedule-drawer" aria-label="Linked schedule details">{selected && <><div className="drawer-title"><div><span className="eyebrow">LINKED SCHEDULE</span><strong>{selected.activity}</strong><small>{selected.resource} · {selected.hours}h/day · {days[selected.start - 1]} → {days[selected.finish - 1]}</small></div><button onClick={() => setSelectedActivity(null)} aria-label="Close schedule details"><X size={15} /></button></div><div className="drawer-timeline"><span>Schedule dates</span><div>{days.map((day, i) => <i className={(i + 1 >= selected.start && i + 1 <= selected.finish && workingDay(i)) ? 'active' : !workingDay(i) ? 'off' : ''} key={day}>{day}</i>)}</div></div><p>Make corrections in the schedule while keeping this resource view open for comparison.</p><button className="primary-button" onClick={() => onOpenSchedule?.(selected.activity)}>Open full schedule</button></>}</div></div><div className="panel-note"><span className="note-icon"><Users size={14} /></span><div><strong>{notice}</strong><span>Hours are calculated from the linked activity schedule. A person assigned to overlapping activities shows combined daily hours and turns red above 8 hours.</span></div></div></section>;
}
function RisksView({ notice }) { const risks = resourceLoads.filter((r) => r.overall > 100).flatMap((resource) => resource.projects.map(([project, value]) => ({ resource: resource.name, project, value, detail: `${resource.name} is assigned at ${resource.overall}% overall; ${value}% is on this project.` }))); return <section className="risks-view"><div className="placeholder-toolbar"><button className="toolbar-button active"><AlertTriangle size={15} /> Resource overload risks</button><span className="range-label">{risks.length} flagged items · management action required</span></div><div className="risk-list">{risks.map((risk) => <article className="risk-card" key={`${risk.resource}-${risk.project}`}><div className="risk-icon"><AlertTriangle size={17} /></div><div><strong>{risk.project} · overloaded resource</strong><span>{risk.detail}</span><p>Project management team needs to restructure assignments before the schedule slips.</p></div><b>{risk.value}% project load</b></article>)}</div><div className="panel-note"><span className="note-icon"><AlertTriangle size={14} /></span><div><strong>{notice}</strong><span>Resource overloads are linked back to the affected project item for coordinated risk management.</span></div></div></section>; }
function ResourcePlaceholderLegacy({ action, notice }) { const resources = [['Database Administrator','$0.00/h','$0.00/h','$0.00','$0.00','IT'],['Network Administrator','$0.00/h','$0.00/h','$0.00','$0.00','IT'],['Business Analysts','$0.00/h','$0.00/h','$0.00','$0.00','People'],['Project Managers','$0.00/h','$0.00/h','$0.00','$0.00','People'],['IT Staff','$0.00/h','$0.00/h','$0.00','$0.00','IT'],['Server Administrator','$0.00/h','$0.00/h','$0.00','$0.00','IT'],['Systems Architect','$0.00/h','$0.00/h','$0.00','$0.00','People'],['Desktop Support','$0.00/h','$0.00/h','$0.00','$0.00','IT'],['Application Deployment','$0.00/h','$0.00/h','$0.00','$0.00','People'],['Team Members','$0.00/h','$0.00/h','$0.00','$0.00','People'],['Business Sponsors','$0.00/h','$0.00/h','$0.00','$0.00','People'],['Application Developers','$0.00/h','$0.00/h','$0.00','$0.00','People']]; return <section className="placeholder-view"><div className="placeholder-toolbar"><button className="toolbar-button active"><Users size={15} /> Resource table</button><span className="range-label">Resource information</span></div></section>; }
