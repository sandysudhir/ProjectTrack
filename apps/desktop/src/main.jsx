...
function DashboardView({ rows, notice }) {
  const projectRows = projects.filter((p) => p.id !== 'all').map((project) => { const items = rows.filter((r) => r.projectId === project.id || (project.id === 'orion' && !r.projectId)); const progress = items.length ? Math.round(items.reduce((sum, r) => sum + r.progress, 0) / items.length) : 0; const risk = items.filter((r) => r.status === 'At risk').length; return { ...project, progress, risk, count: items.length }; });
