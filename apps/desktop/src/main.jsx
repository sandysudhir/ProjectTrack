...
function ResourcePlaceholder({ action, notice, projectLabel = 'All Projects' }) { const visible = projectLabel === 'All Projects' ? resourceLoads : resourceLoads.map((r) => ({ ...r, projects: r.projects.filter(([p]) => p === projectLabel), overall: r.projects.filter(([p]) => p === projectLabel).reduce((sum, [, value]) => sum + value, 0), hours: Math.round(r.hours * r.projects.filter(([p]) => p === projectLabel).reduce((sum, [, value]) => sum + value, 0) / Math.max(1, r.overall)) })).filter((r) => r.overall > 0); const overloaded = visible.filter((r) => r.overall > 100); return <section className="resource-view"><div className="placeholder-toolbar"><button className="toolbar-button active"><Users size={15} /> Resource load</button><span className="range-label">{projectLabel} · {visible.length} resources · overall utilization</span></div><div className="resource-summary"><div><span>Average utilization</span><strong>{Math.round(visible.reduce((s, r) => s + r.overall, 0) / Math.max(1, visible.length))}%</strong></div><div><span>Overloaded resources</span><strong className={overloaded.length ? 'risk-text' : ''}>{overloaded.length}</strong></div><div><span>Risk items</span><strong className={overloaded.length ? 'risk-text' : ''}>{overloaded.length}</strong></div></div><div className="resource-load-list">{visible.map((resource) => <article className={resource.overall > 100 ? 'resource-load overloaded' : 'resource-load'} key={resource.name}><div className="resource-load-head"><div><strong>{resource.name}</strong><span>{resource.category} · {resource.projects.map(([project]) => project).join(' · ')}</span></div><b>{resource.overall}%</b></div><div className="resource-metrics"><span><small>Planned hours</small><strong>{resource.hours} h</strong></span><span><small>Engagement</small><strong>{resource.overall}%</strong></span><span><small>Activities</small><strong>{resource.activities.length}</strong></span></div><div className="load-track"><i style={{ width: `${Math.min(100, resource.overall)}%` }} /><em style={{ left: `${Math.min(100, resource.overall)}%` }} /></div>{resource.overall > 100 && <div className="overload-note"><AlertTriangle size={14} /> Overloaded by {resource.overall - 100}% · restructure assignments across {resource.projects.map(([project]) => project).join(' and ')}</div>}<div className="resource-project-splits">{resource.projects.map(([project, value]) => <span key={project}><small>{project}</small><b>{value}%</b></span>)}</div><div className="resource-activities"><small>Assigned activities taking up this time</small>{resource.activities.map((activity) => <span key={activity}><ClipboardList size={12} />{activity}</span>)}</div></article>)}</div><div className="panel-note"><span className="note-icon"><Users size={14} /></span><div><strong>{notice}</strong><span>Each resource shows planned hours, engagement percentage and the activities consuming that capacity. Utilization above 100% is an overload risk.</span></div></div></section>; }
