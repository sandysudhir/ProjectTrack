...
function ResourcePlaceholder({ action, notice, projectLabel = 'All Projects', rows = [] }) {
  ...
  return <section className="resource-view"><div className="placeholder-toolbar"><button className="toolbar-button active"><Users size={15} /> Daily resource schedule</button><span className="range-label">{projectLabel} · daily capacity tracker · 8h baseline</span></div><div className="resource-summary"><div><span>Average daily load</span><strong>{Math.round(resourceData.reduce((sum, r) => sum + r.daily.reduce((a,b) => a+b,0) / Math.max(1,r.daily.length),0) / Math.max(1,resourceData.length))} h</strong></div><div><span>Overloaded resources</span><strong className={overloaded.length ? 'risk-text' : ''}>{overloaded.length}</strong></div><div><span>Schedule-linked activities</span><strong>{resourceData.reduce((sum,r) => sum + r.assignments.length, 0)}</strong></div></div><div className="resource-grid-wrap"><div className="resource-grid-head"><span>Resource / assigned activity</span>{days.map((day) => <span key={day}>{day}</span>)}</div>{resourceData.map((resource) => <div className="resource-group" key={resource.name}><div className="resource-group-title"><strong>{resource.name}</strong><span>{resource.category} · {resource.overall}% overall · {resource.hours} planned h</span></div><div className="resource-total-row"><b>Daily utilization</b>{resource.daily.map((hours, i) => <span className={hours > 8 ? 'overload-cell' : hours ? 'loaded-cell' : ''} key={days[i]}>{hours}h<br/><small>{Math.round(hours / 8 * 100)}%</small></span>)}</div>{resource.assignments.map((assignment) => <div className="resource-assignment-row" key={assignment.activity}><span>↳ {assignment.activity}<small>{assignment.hours}h/day · schedule linked</small></span>{days.map((_, i) => <span className={i + 1 >= assignment.start && i + 1 <= assignment.finish ? 'assignment-cell' : ''} key={i}>{i + 1 >= assignment.start && i + 1 <= assignment.finish ? `${assignment.hours}h` : ''}</span>)}</div>)}<div className="resource-load-track"><i style={{ width: `${Math.min(100, resource.overall)}%` }} /><b>{resource.overall}% engagement</b></div>{resource.daily.some((hours) => hours > 8) && <div className="overload-note"><AlertTriangle size={14} /> Daily usage exceeds 8 hours. Rebalance assignments; this is a schedule risk.</div>}</div>)}</div><div className="panel-note"><span className="note-icon"><Users size={14} /></span><div><strong>{notice}</strong><span>Hours are calculated from the linked activity schedule. A person assigned to overlapping activities shows combined daily hours and turns red above 8 hours.</span></div></div></section>;
}
