...
const resourceLoads = [
  { name: 'Systems Architect', category: 'People', overall: 128, projects: [['Orion Program', 80], ['On2Cook B2C', 48]] },
  { name: 'Industrial Designer', category: 'People', overall: 92, projects: [['On2Cook B2C', 62], ['Atlas Program', 30]] },
  { name: 'Application Developer', category: 'People', overall: 105, projects: [['On2Cook.AI', 70], ['Delta Release', 35]] },
  { name: 'Business Analyst', category: 'People', overall: 64, projects: [['Orion Program', 40], ['Atlas Program', 24]] },
  { name: 'Project Manager', category: 'People', overall: 78, projects: [['Orion Program', 35], ['Atlas Program', 25], ['Delta Release', 18]] },
  { name: 'Contractor Procurement', category: 'External', overall: 118, projects: [['Atlas Program', 68], ['On2Cook B2C', 50]] },
];
function ResourcePlaceholder({ action, notice, projectLabel = 'All Projects' }) { const visible = projectLabel === 'All Projects' ? resourceLoads : resourceLoads.map((r) => ({ ...r, projects: r.projects.filter(([p]) => p === projectLabel), overall: r.projects.filter(([p]) => p === projectLabel).reduce((sum, [, value]) => sum + value, 0) })).filter((r) => r.overall > 0); const overloaded = visible.filter((r) => r.overall > 100); return <section className="resource-view"><div className="placeholder-toolbar"><button className="toolbar-button active"><Users size={15} /> Resource load</button><span className="range-label">{projectLabel} · {visible.length} resources · overall utilization</span></div><div className="resource-summary"><div><span>Average utilization</span><strong>{Math.round(visible.reduce((s, r) => s + r.overall, 0) / Math.max(1, visible.length))}%</strong></div><div><span>Overloaded resources</span><strong className={overloaded.length ? 'risk-text' : ''}>{overloaded.length}</strong></div><div><span>Risk items</span><strong className={overloaded.length ? 'risk-text' : ''}>{overloaded.length}</strong></div></div><div className="resource-load-list">{visible.map((resource) => <article className={resource.overall > 100 ? 'resource-load overloaded' : 'resource-load'} key={resource.name}><div className="resource-load-head"><div><strong>{resource.name}</strong><span>{resource.category} · {resource.projects.map(([project]) => project).join(' · ')}</span></div><b>{resource.overall}%</b></div><div className="load-track"><i style={{ width: `${Math.min(100, resource.overall)}%` }} /><em style={{ left: `${Math.min(100, resource.overall)}%` }} /></div>{resource.overall > 100 && <div className="overload-note"><AlertTriangle size={14} /> Overloaded by {resource.overall - 100}% · restructure assignments across {resource.projects.map(([project]) => project).join(' and ')}</div>}<div className="resource-project-splits">{resource.projects.map(([project, value]) => <span key={project}><small>{project}</small><b>{value}%</b></span>)}</div></article>)}</div><div className="panel-note"><span className="note-icon"><Users size={14} /></span><div><strong>{notice}</strong><span>Utilization above 100% is automatically treated as a resource overload risk and is surfaced to project managers.</span></div></div></section>; }
