...
const ribbonGroups = {
  Home: [['Schedule', Table2], ['Arrow', ArrowDownToLine], ['Select', Grid3X3], ['Bar', Plus], ['Revise', RefreshCw], ['Link', Link2], ['Arrange', SlidersHorizontal], ['Autofit', Columns3]],
  Insert: [['Task', Plus], ['Milestone', Gauge], ['Text box', FileText], ['Picture', PanelLeft], ['Object', Database], ['Summary graph', BarChart3], ['Timescale', Clock3], ['Datelines', Clock3]],
  Format: [['Bar style', SlidersHorizontal], ['Critical path', Activity], ['Link', Link2], ['Datelines', Clock3], ['Gridlines', Grid3X3], ['Header & footer', FileText]],
  View: [['Summary bars', BarChart3], ['Action columns', Columns3], ['Show level', ListFilter], ['Density', Gauge], ['Links', Link2], ['Restore all', RefreshCw]],
  Project: [['Project info', FolderKanban], ['Activity info', ClipboardList], ['Resource info', Users], ['Work calendars', CalendarDays], ['Timeline units', Clock3], ['Layouts', Grid3X3]],
  Tools: [['FastSteps', Gauge], ['Define', ClipboardList], ['Get updates', ArrowDownToLine], ['Save baseline', ArrowDownToLine], ['Clear baseline', X], ['Find', Search], ['Filters', ListFilter]],
  Application: [['On the Web', ArrowUpToLine], ['About', CircleHelp], ['Keyboard shortcuts', HelpCircle], ['Tab workspace', Grid3X3], ['Status bar', Activity], ['Themes', SlidersHorizontal], ['Toolbars', Columns3], ['Switch windows', MoreHorizontal]],
};
const initialParams = new URLSearchParams(window.location.search);
const futureCommands = new Set(['Revise', 'Link', 'Save baseline', 'Clear baseline', 'Reset revised', 'Reset actual', 'Column map', 'Get updates', 'Cost report', 'Resource cost report']);
const dialogFields = {
  ...
  'Activity info': ['Row tab', 'Bars tab', 'Bar ID', 'Duration', 'Fixed Duration', 'Scheduled Start Date', 'Scheduled Finish Date', '% Complete', 'Priority', 'Constraint Type', 'Calendar'],
  'Resource info': ['Resource Name', 'Category', 'Per Use Cost', 'Initials', 'Standard Rate', 'Overtime Rate', 'Code', 'Group', 'Full Name', 'Job Title', 'Employee ID', 'Company', 'Department', 'Resource Notes', 'Material Label'],
  'Calendars': ['Calendar selector', 'New', 'Delete', 'Copy Calendar', 'Paste Calendar', 'Base Calendar', 'Month / year', 'Date grid', 'Exceptions', 'Create Exception', 'Work Shift Details', 'Typical week daily hours', 'Copy Day', 'Paste Day', 'Clear Day'],
...
function App() {
  ...
  const visibleRows = expanded ? filteredRows : filteredRows.filter((row) => row.level === 0 || row.type === 'milestone');
    ...
    <div className="work-area">
      ...
      <main className="main-content">
        ...
        {mode === 'loading' ? <div className="state-panel"><RefreshCw size={25} className="spin" /><h2>Loading workspace</h2><p>Preparing the deterministic sample fixture…</p></div> : <>
          {showCalendar ? <CalendarPlaceholder notice={notice} action={action} /> : showResource ? <ResourcePlaceholder notice={notice} action={action} /> : <ScheduleView rows={visibleRows} leftWidth={leftWidth} setLeftWidth={setLeftWidth} density={density} setDensity={setDensity} zoom={zoom} setZoom={setZoom} expanded={expanded} action={action} notice={notice} />}
        </>}
      </main>
    </div>
...
function ScheduleView({ rows, leftWidth, setLeftWidth, density, setDensity, zoom, setZoom, expanded, action, notice }) {
  ...
  return <section className="schedule-panel"><div className="panel-toolbar"><div className="toolbar-group"><button className="toolbar-button active"><Table2 size={15} /> Grid + Gantt</button><button className="toolbar-button" onClick={() => action('Show level')}><ListFilter size={15} /> Show level <ChevronDown size={13} /></button><button className="toolbar-button" onClick={() => action('Active now')}><Play size={14} /> Active now</button><button className="toolbar-button" onClick={() => action('Projects only')}>Projects only</button><button className="toolbar-button" onClick={() => action('Expand everything')}>Expand everything</button></div><div className="toolbar-group"><label className="density-control">Row height <select value={density} onChange={(e) => setDensity(e.target.value)}><option>Compact</option><option>Comfortable</option><option>Expanded</option></select></label><label className="zoom-control">Timeline <input type="range" min="60" max="160" value={zoom} onChange={(e) => setZoom(Number(e.target.value))} /><span>{zoom}%</span></label></div></div><div className="schedule-body"><div className="grid-pane" style={{ width: `${leftWidth}%` }}><div className="grid-header"><span className="row-number">#</span><span className="activity-name">Activity name</span><span>Start</span><span>Finish</span><span>Dur.</span><span>Status</span></div><div className="grid-rows">{rows.map((row) => <div className={`grid-row ${row.type === 'summary' ? 'summary' : ''} ${row.type === 'milestone' ? 'milestone-row' : ''}`} key={row.id} style={{ '--indent': `${row.level * 18}px` }}><span className="row-number">{row.id}</span><span className="activity-name" style={{ paddingLeft: `calc(10px + ${row.level * 18}px)` }}>{row.level > 0 && <span className="branch-line" />}{row.type === 'summary' ? <ChevronDown size={13} /> : row.type === 'milestone' ? <Gauge size={12} className="milestone-icon" /> : <span className="task-dot" />}<span>{row.name}</span></span><span>{row.start}</span><span>{row.finish}</span><span>{row.duration}</span><span className={`status ${row.status === 'At risk' ? 'risk' : row.status === 'Complete' ? 'complete' : ''}`}>{row.status}</span></div>)}</div></div><button className="splitter" aria-label="Resize grid pane" onClick={() => setLeftWidth(leftWidth > 44 ? 34 : 52)}><span /></button><div className="timeline-pane"><div className="timeline-header"><div className="month-label">September 2026</div><div className="timeline-date-stack"><div className="weekday-labels">{timeline.map((day, i) => <span key={`w-${day}`}>{['T','W','T','F','S','S','M'][i % 7]}</span>)}</div><div className="day-labels">{timeline.map((day, i) => <span key={day} className={i === 8 ? 'today' : ''}>{day}</span>)}</div></div></div><div className="timeline-grid" style={{ '--scale': scale }}><div className="today-line" style={{ left: `${(8.5 / 30) * 100}%` }}><span>Today</span></div>{rows.map((row, index) => <div key={row.id} className={`timeline-row ${row.type === 'summary' ? 'summary' : ''}`}><div className={`bar ${row.color}`} style={{ left: `${(row.bar[0] / 30) * 100}%`, width: row.type === 'milestone' ? '0' : `${((row.bar[1] - row.bar[0] + 1) / 30) * 100 * scale}%` }}>{row.type === 'milestone' ? <span className="diamond" /> : <><span className="bar-progress" style={{ width: `${row.progress}%` }} /><span className="bar-label">{row.name}</span>{index > 0 && <span className="dependency" />}</>}</div></div>)}</div><div className="timeline-footer"><span>01 Sep</span><span>Week 37</span><span>08 Sep</span><span>15 Sep</span><span>22 Sep</span><span>30 Sep</span></div></div></div><div className="panel-note"><span className="note-icon"><Clock3 size={14} /></span><div><strong>UI-01 shell fixture</strong><span>Navigation and visual structure are active. Scheduling commands remain intentionally outside this phase.</span></div><button onClick={() => action('Open validation note')}>Details</button></div></section>;
}
