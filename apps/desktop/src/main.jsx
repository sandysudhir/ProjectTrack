    ...
    <div className="work-area">
      ...
      <main className="main-content">
        ...
        {mode === 'loading' ? <div className="state-panel"><RefreshCw size={25} className="spin" /><h2>Loading workspace</h2><p>Preparing the deterministic sample fixture…</p></div> : <>
          {showFiles ? <ProjectFilesView projectId={activeProject} mode={activeNav} files={projectFiles} onAddFile={addProjectFile} onAddComment={addProjectComment} notice={notice} /> : showCost ? <CostLayoutView notice={notice} action={action} /> : showStatus ? <StatusLayoutView rows={filteredRows} notice={notice} /> : showCalendar ? <CalendarPlaceholder notice={notice} action={action} /> : showResource ? <ResourcePlaceholder notice={notice} action={action} /> : <ScheduleView rows={visibleRows} leftWidth={leftWidth} setLeftWidth={setLeftWidth} density={density} setDensity={setDensity} zoom={zoom} setZoom={setZoom} outlineMode={outlineMode} setOutlineMode={(next) => { setOutlineMode(next); setExpanded(next !== 'compact'); }} action={action} notice={notice} activeTool={activeTool} onCreateBar={(day) => createBar({ projectId: activeProject, day })} onToggleCollapse={toggleCollapse} collapsedIds={collapsedIds} />}
        </>}
      </main>
    </div>
